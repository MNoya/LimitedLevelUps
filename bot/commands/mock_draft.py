...
A lobby is closed by anyone from the pod Settings panel, or by the manager's own inactivity watch once it
sits under-filled with nobody new joining the Draftmancer session for `pod_idle_offer_minutes`, which
first offers a Keep it Open card before closing. Both delete the event, so a channel never accumulates
lobbies that were never drafted.
"""
