...
async def launch_from_signal(
    ...
    await add_thread_watcher(thread)
...
async def add_thread_watcher(thread: discord.Thread) -> None:
    ...
    try:
        ...
        log.warning(f"could not add watcher {watch_id} to pod thread {thread.id}", exc_info=True)
