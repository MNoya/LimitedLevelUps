...
from bot.models import PodDraftEvent, PodScheduleSlot, PodSignal, PodSignalMember
...
def test_a_slot_rolls_to_tomorrow_once_todays_start_has_passed(slot_key, called_at, expected_start):
    start = next_slot_start(slot_key, called_at)

    assert start == expected_start.replace(tzinfo=SCHEDULE_TZ)


def test_buckets_of_one_time_share_a_slot_across_the_week():
    friday_late = bucket_for_slot(FRIDAY, SLOT_LATE)
    saturday_late = bucket_for_slot(SATURDAY, SLOT_LATE)

    assert friday_late.key != saturday_late.key
    assert slot_of(friday_late.key) == slot_of(saturday_late.key) == SLOT_LATE
...
def test_a_rolled_column_stays_one_column_across_the_weekend_boundary():
    ...
    slot_keys = _slot_order(slots)

    assert slot_keys == [SLOT_LATE]
    assert [slot.set_code for slot in _key_slots(slots, SLOT_LATE)] == [LATEST, LATEST]
...
def test_a_dead_slot_is_dropped_once_the_column_rolled_past_it():
    ...
    kept = pod_launch._without_rolled_past_slots(key_slots)
...
def test_a_dead_slot_stays_while_the_column_has_not_rolled():
    key_slots = [_gathering("EARLY", FRIDAY, status=STATUS_EXPIRED, count=1)]

    assert pod_launch._without_rolled_past_slots(key_slots) == key_slots


def test_a_played_pod_is_never_dropped_by_a_roll():
    key_slots = [_played("LATE", FRIDAY), _gathering("EVENING", SATURDAY)]
...
def test_only_the_last_table_of_a_split_carries_the_signup_button():
    ...
    key_slots = [first, second]

    items = [_slot_item(slot, None, key_slots) for slot in key_slots]
...
def test_the_morning_post_opens_one_pod_per_format_the_day_offers(session, latest_and_peasant):
    ...
    early, late = bucket_for_slot(FRIDAY, SLOT_EARLY).key, bucket_for_slot(FRIDAY, SLOT_LATE).key
...
def test_a_drafting_pod_leaves_the_other_format_at_its_slot_gathering(session, monkeypatch, latest_and_peasant):
    ...
    early_column = [slot for slot in slots if slot_of(slot.bucket_key) == SLOT_EARLY]
...
def test_both_tables_of_a_split_pod_reach_the_column(session, monkeypatch, latest_only):
    monkeypatch.setattr("bot.services.pod_launch.SessionLocal", _session_factory(session))
    slot_time = slot_event_time(FRIDAY, bucket_for_slot(FRIDAY, SLOT_EARLY).key)
...
def test_an_off_grid_pod_reflects_as_a_bonus_slot(session, monkeypatch, latest_only):
    ...
    bonus = [slot for slot in slots if slot_of(slot.bucket_key) == SLOT_BONUS]
...
def test_a_slot_looks_at_tomorrow_for_a_pod_its_closed_slot_can_never_open(session, monkeypatch):
    """The Set Championship sits on a slot the format schedule closes, so no signal ever reaches its day and
    the slot_key it holds would never look there. The launcher is what players read the day before, so a pod
    that already exists at tomorrow's slot is found without one. The slot_key with nothing tomorrow stays put."""
    monkeypatch.setattr("bot.services.pod_launch.SessionLocal", _session_factory(session))
    session.add(PodScheduleSlot(day=SATURDAY, slot=SLOT_EARLY, formats=[], source="manual"))
    session.flush()
    afternoon = bucket_for_slot(SATURDAY, SLOT_EARLY).key
    _seed_pod(
    ...
    early = [slot for slot in slots if slot_of(slot.bucket_key) == SLOT_EARLY]
    late = [slot for slot in slots if slot_of(slot.bucket_key) == SLOT_LATE]
    assert [(slot.championship, slot.slot_time) for slot in early if slot.committed] == [
        (True, slot_event_time(SATURDAY, afternoon)),
    ]
    assert [slot.slot_time for slot in late] == [slot_event_time(FRIDAY, bucket_for_slot(FRIDAY, SLOT_LATE).key)]


def test_a_closed_slot_opens_no_pod_while_the_other_slot_still_does(session):
    session.add(PodScheduleSlot(day=FRIDAY, slot=SLOT_EARLY, formats=[], source="manual"))
    session.flush()
...
def test_rolling_into_a_closed_slot_opens_nothing(session, monkeypatch):
    monkeypatch.setattr("bot.services.pod_launch.SessionLocal", _session_factory(session))
    session.add(PodScheduleSlot(day=SATURDAY, slot=SLOT_LATE, formats=[], source="manual"))
    session.flush()

    rolled = pod_launch.roll_slot_forward_sync(
        slot_key=SLOT_LATE, from_day=FRIDAY, guild_id="g", channel_id="c", message_id="today",
    )

    assert rolled == []


def test_rolling_a_slot_opens_the_next_day_and_is_idempotent(session, monkeypatch, latest_only):
    monkeypatch.setattr("bot.services.pod_launch.SessionLocal", _session_factory(session))
    ...
    time_key = bucket_for_slot(SATURDAY, SLOT_LATE).key
...
def test_rolling_a_slot_opens_every_format_the_next_day_offers(session, monkeypatch, latest_and_peasant):
    monkeypatch.setattr("bot.services.pod_launch.SessionLocal", _session_factory(session))
...
def test_a_rolled_slot_and_todays_slot_share_the_message_and_the_snapshot_stacks_them(
    ...
    late_column = [slot for slot in slots if slot_of(slot.bucket_key) == SLOT_LATE]
    assert [slot.slot_time for slot in late_column] == [
        slot_event_time(FRIDAY, "LATE"), slot_event_time(SATURDAY, bucket_for_slot(SATURDAY, SLOT_LATE).key),
    ]
...
def test_a_fired_slot_with_no_pod_left_renders_closed(session, monkeypatch, latest_only):
    ...
    late_column = [
        ...
        if slot_of(entry.bucket_key) == SLOT_LATE
    ]
...
@pytest.mark.parametrize("played_slot, expected_slot", [(SLOT_EARLY, SLOT_EARLY), (SLOT_LATE, SLOT_LATE)])
def test_play_again_offers_the_slot_a_day_after_the_pod_played(
    session, monkeypatch, latest_only, played_slot, expected_slot,
):
    monkeypatch.setattr("bot.services.pod_launch.SessionLocal", _session_factory(session))
    _seed_signal(session, bucket_for_slot(SATURDAY, SLOT_EARLY).key, SATURDAY, message_id="tomorrow")
    _seed_signal(session, bucket_for_slot(SATURDAY, SLOT_LATE).key, SATURDAY, message_id="tomorrow")
    played_at = slot_event_time(FRIDAY, bucket_for_slot(FRIDAY, played_slot).key)
    event = _seed_pod(session, played_at, set_code=LATEST)
    session.commit()

    keys = pod_launch.play_again_slots_sync(event.id, played_at)

    expected = bucket_for_slot(SATURDAY, expected_slot).key
...
def test_play_again_offers_an_off_grid_pod_the_nearest_slot_a_day_later(
    ...
    _seed_signal(session, bucket_for_slot(SATURDAY, SLOT_EARLY).key, SATURDAY, message_id="tomorrow")
    _seed_signal(session, bucket_for_slot(SATURDAY, SLOT_LATE).key, SATURDAY, message_id="tomorrow")
    late_start = slot_event_time(FRIDAY, bucket_for_slot(FRIDAY, SLOT_LATE).key)
    ...
    assert [time_key_of(key) for key in keys] == [bucket_for_slot(SATURDAY, SLOT_LATE).key]


def test_play_again_skips_a_mock_draft(session, monkeypatch, latest_only):
    monkeypatch.setattr("bot.services.pod_launch.SessionLocal", _session_factory(session))
    _seed_signal(session, bucket_for_slot(SATURDAY, SLOT_EARLY).key, SATURDAY, message_id="tomorrow")
    played_at = slot_event_time(FRIDAY, bucket_for_slot(FRIDAY, SLOT_EARLY).key)
    event = _seed_pod(session, played_at, set_code=LATEST)
    event.kind = "mock"
    session.commit()
...
def _set_slots(session, days, formats):
    for day in days:
...
@pytest.fixture
def latest_only(session):
    _set_slots(session, (FRIDAY, SATURDAY), (schedule.LATEST,))
