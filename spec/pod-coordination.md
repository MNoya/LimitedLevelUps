...
This document is a reference for agents working on the pod system: it walks the pod-draft workflow end to end. It is not authoritative. Where it and the code disagree the code is right, and it may lag behind recent changes.
...
- Every day: Early Pod at 2 PM ET, Late Pod at 9 PM ET.
...
A pod opens its thread at 5 players, which gives the room a place to line up a sixth before the draft starts.

When a pod reaches 5 people, it opens for real. The bot posts a signup card with its own thread, and everyone who clicked that pod is carried over as a Yes. The other formats at that time keep collecting behind their own buttons. A pod for a later day waits: it collects signups overnight and opens on the morning of its own day, so nothing starts hours early.
...
- **Leave** — you are out, and you come off the list.
...
- The bot posts a reminder that shows the roster as the tables it makes. It carries Confirm and Leave buttons to say whether you are still playing. There is no Maybe at this point: you are confirming a yes or a no. Confirming stops at the start time: the tables are built by then and the room is open, so a press after it answers that the pod has started and records nothing. Signing up and leaving stay open to the end, and the Join Draft button in the thread is the way into a pod that is already drafting.
...
When enough players are in the lobby, anyone can start a ready check from the lobby card in the thread. The bot only offers a ready check button of its own once the room is full or everyone who said yes is in, so it never puts one in front of a table that could still start on top of someone who is on their way. While it is still waiting on a confirmed player, it says so in the thread and names them, and points you at the lobby card to start the check yourself when you decide not to wait. The bot posts a card of its own carrying I'm Ready, Not Ready and Stop, and the Draftmancer prompt pops up at the same time. Answer on whichever is in front of you, it counts either way. The card shows who has answered and who has not, and the draft starts by itself the moment the last seat says I'm Ready. A full table of 8 starts as soon as everyone is ready.
...
A lobby that is one or two players short is the most common way a pod fails to fire. Anyone can write `!pod` in any channel to ask for them. Start the message with it and the rest is yours to write, so `!pod anyone free tonight` works. Write a set or cube code anywhere in the message and it asks for that format, so `!pod any DOM enjoyers in chat?` calls for the DOM pod even when another one is closer to firing. Name more than one and it tries each in the order you wrote them, and `cube` asks for whichever cube the day is running. A format nobody is drafting right now gets you the normal answer, so you still learn which pod is live. Your message stays up and the bot posts one line under it: which pod is open, how many players it still needs, how many are already sitting in the Draftmancer lobby, and a link to the thread. The line carries a Join Draft button that hands back a Draftmancer link with your Arena name already filled in, so you arrive matched to your leaderboard profile. The lobby card lists `!pod` in its Commands block, so the pod that needs players names the way to ask for them.
...
Organizers have two controls on the roster card while it is asking for confirmations. 📋 opens a private list with **Confirmed Players** and **Declined Players**, for recording what somebody said in chat but never pressed. While a large pod is holding for a split it also carries **Feature On Table 1**, which pins the picked players to the first table so the release seats them there and the rest fill in signup order. 🚀 **Open Tables** ends the wait early and deals the tables there and then. Both edits are posted in the thread naming who made them and who they were about, since the room is waiting on those answers. After the split there is no table-swap control: anyone who wants a different table sorts it out in chat.

You do not have to go looking for your table. Each table's lobby post opens by naming which table it is and everybody on it, grouped by the answer they gave, and the list in the main thread links to every table. That post carries the table's own link beside the **Join Draft** button, which hands you a personal link and sends you to the table with a seat for you even when you press it in the wrong thread.
...
Report your results in the thread as you play. In an 8-player or 10-player pod each round also DMs you your pairing with the same dropdown. `/report-results` works anywhere, including in a DM with the bot, and pulls up a private card holding every match you still owe, so a missed DM never stops you from reporting. In a team draft all three of your matches are open from the moment the draft ends, so the card lists all three and you report whichever you played. A match whose players are still finishing an earlier round carries an hourglass and the round it waits on, so the board never reads as ready when only part of a round is. Winning games earns pod points on the leaderboard, so a good run in a pod moves you up the standings the same as a strong ladder result. A trophy is a 3-0, so winning a pod on a 2-1 is first place without one. Pod results are always public: you do not need to opt in, and playing pods is enough to appear on the board.

Share your deck in the last round. Post a screenshot of it in the thread, and submit your deck colors with the Submit Deck button or with `/report-results`. You can write the deck description right on the screenshot or as a separate message just before or after it, and the bot pairs a bare screenshot with your nearby message as its caption. If you post more than one screenshot, react to the right one with the camera emoji and the bot uses that image. While the last tables are still playing, the bot posts one plain reminder in the thread if anyone who finished has not posted a screenshot yet, with nobody pinged. Five minutes after the last result of the pod, it mentions the players who still owe a screenshot or their colors. The pod result posts in the channel once the players it showcases have both, and ten minutes after the last result at the latest.

If you have to leave partway through, tell an Organizer so they can drop you. Every match you have left is then a bye for whoever you were going to play: they take the win without playing it, and a 3-0 that holds a bye is a 3-0 like any other. You keep the points for the matches you did play, and the rounds you were not there for stay off your record, so leaving after two wins still reads 2-0. Dropping cannot be undone. This is different from No Match Played, which is for the last round when both players agree to skip a match that decides nothing, and gives neither of them anything. A drop that lands after one turns it into a bye, so telling an Organizer late still gets your opponent the win.
...
Once the pod result is up, its thread signs off with a thank-you, a link to that post, and a **Play Again** button that signs you up for the same time slot and format on the next day, whenever that format is on the next day's schedule. A pod played outside the launcher's slots is invited to the slot closest to a day after it played. The sign-off waits for the result post, so it is the last thing the thread says, and it goes out anyway a few minutes later if no result post ever lands. The result also goes on that day's retired launcher, an On This Day list of every pod the day played with its winner, whose name links to that pod's page on the website.
...
Max Players sets the table size: 6, 8, or 10 seats. A six-seat table asks that lobby to start as soon as six players are in it, and the ready check is where it offers the 3v3 Team Draft. A cap below the players already in the Draftmancer lobby is refused, so shrinking a table that filled up means removing someone first with Kick Player.

If someone loses their connection mid-draft, the draft stops on Draftmancer until they come back. The bot posts a card naming who went and counting down to the vote that opens if they stay gone. A player who comes back inside those two minutes is never asked about: a second card says they are back and the draft carries on. A connection that keeps dropping and returning does not pile those cards up, since the previous pair is removed when the next drop is announced. Only the latest drop and return stay in the thread, and a stall that got as far as the vote is always kept.
