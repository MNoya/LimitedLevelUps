...
- **In-lobby Format Vote** (`pod_format_poll.py`) and the format-driven second-table split (`assess_format_split`, `pick_second_table`) are only reachable from a flex pod, so neither fires today. `/pod-table` still opens a second table on a format a mod picks.
