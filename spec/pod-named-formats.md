...
- **Buttons are `DynamicItem`s** (`SlotToggleButton`, `SlotRsvpButton`, `PlayAgainButton`) registered in `main.py`, since the keys carry formats and no startup registration could enumerate them. Play Again re-signs for the same slot and format, and is only offered when the next day carries that format.
