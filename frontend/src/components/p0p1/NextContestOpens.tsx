...
export function NextContestOpens({ next, showSet = true }: { next: FeaturedContest["next"]; showSet?: boolean }) {
  const now = useNow(1000);
  if (!next || next.previewsOpen.getTime() <= now) return null;

  return (
    <span className="flex items-center justify-center gap-x-3">
      {showSet && <SetGlyph code={next.code} size={17} className="text-white" />}
      {showSet && <span className="font-semibold text-text">{next.code}</span>}
      <span>{showSet ? "picks open" : "Picks open"}</span>
      <span
