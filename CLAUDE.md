...
- **Tests target logic, not framework behavior, and stay few.** Aggregator / scoring / branch logic, fewest cases that cover it. None for a one-key fix, a library's own signature, or values just written — verify those by running the thing.
