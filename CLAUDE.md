...
`spec/archive/` is frozen archaeology: what a feature was meant to do at the time it was built. Read the code for how it works today. Write a spec when asked for one.
...
- **Short user-facing labels: sentence case, capitalize named things.** Help entries, command blurbs, button/select/menu labels, section labels and headings capitalize the first word and any named thing (the specific product, feature, format or object you point at); generic words stay lowercase. "Link the Website", "How Team Draft works", "Confirm your seat in a Pod thread", "Peasant Cube on CubeCobra". Full-sentence notices stay sentence case per the plain declarative rule above; mid-sentence emphasis caps are a separate convention.
- **Code comments: default to none, and docstrings are in scope for every rule here.** If a comment or docstring runs longer than one line, condense it to a single line that keeps the essential point; delete it only if one line can't justify it. The code is already self-explanatory if names are right. No periods at the end of a single-line comment or docstring (they're labels, not sentences). No parenthetical asides. Don't paraphrase library / decorator behavior at the declaration site — that belongs in upstream docs, not your file.
