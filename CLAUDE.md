...
- **When a pod behavior change ships, update `docs/guide/pod-coordination.md` in the same commit.** It's the plain-language player guide describing only live behavior, headed for a Discord guide page or the website, so it must never describe a state the bot isn't in. It is the one document outside this file that is kept current.
