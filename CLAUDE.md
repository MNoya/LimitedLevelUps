...
- **Code comments: default to none, and docstrings are in scope for every rule here.** If a comment or docstring runs longer than one line, delete the whole block — don't shrink it, delete it. The code is already self-explanatory if names are right. No periods at the end of a single-line comment or docstring (they're labels, not sentences). No parenthetical asides. Don't paraphrase library / decorator behavior at the declaration site — that belongs in upstream docs, not your file.
