...
- **Every website change ends with how to look at it.** Close the reply with a `**Check on the site:**` line: the command to run, the exact path to open, what to click to reach the changed state, and the `VITE_DATA_MODE` to use when the default won't show it. Don't drive a browser to verify it yourself unless told to; the user opens the page. A change touching both carries both lines, the Discord one last.
