...
- **No period ends a line of user-facing copy.** Buttons, labels, headings, notices, subtext, and every line of a multi-line message, including a head line sitting above a `-#` subtext line. Inside a line, punctuate normally: a period splitting two sentences stays, and so does one fencing a sentence off from a trailing link. Two things keep their period: a line ending on an abbreviation like `etc.`, and real prose (spec docs, Server Guide pages, the site's explainer copy), which is paragraphs and not microcopy. Never restructure a message to dodge a period, and never collapse two sentences into a comma splice to lose one. Two sentences stay two sentences, or the second becomes a `-#` subtext line. Prefer one sentence per message and cut a second that only restates the first. Covers the bot and the site.
